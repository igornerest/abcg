...
glm::vec4 OpenGLWindow::getRandomVertexColor() {
  ...
  return glm::vec4{color_rd(m_randomEngine), color_rd(m_randomEngine), color_rd(m_randomEngine), 1.0f}; 
}
...
void OpenGLWindow::paintUI() {
  ...
  {
    auto widgetSize{ImVec2(280, 170)};
    ...
    // Edit vertex colors
    auto colorEditFlags{ImGuiColorEditFlags_NoTooltip | ImGuiColorEditFlags_NoPicker};

    ImGui::PushItemWidth(215);
    
    ImGui::Checkbox("Flat Color", &m_displayFlatColor);
    ImGui::Checkbox("Random Color", &m_diplayRandomColor);

    if (m_displayFlatColor) {
    ...
    ImGui::SliderFloat("Delay",  &m_renderDelay, 0.0f, 500.0f, "%.0f ms");
    ImGui::PopItemWidth();
...
void OpenGLWindow::setupModel() {
  ...
  for (size_t i = 0; i < 3; i++) {
    colors.push_back(m_displayFlatColor ?  m_flatColor : m_vertexColors[i]);
  }
